...
def discover_table_layout(
    ...
    """Locate and validate one layer's PLE shards in a safetensors checkpoint.
    ...
    if missing or unexpected:
        raise ValueError(
            f"PLE shards of layer {layer_index} in {model_dir} do not cover "
...
class MappedTable:
    ...
    def __init__(self, layout: TableLayout, device: torch.device) -> None:
        ...
        self.views = [
            self._maps[s.path][1][s.offset : s.offset + s.rows * self.row_bytes].reshape(
                s.rows, self.row_bytes
            )
    ...
    @classmethod
    def zeros(cls, num_rows: int, row_bytes: int, device: torch.device) -> "MappedTable":
        ...
        mapping = mmap.mmap(-1, max(num_rows * row_bytes, _PAGE), prot=mmap.PROT_READ)
        array = np.frombuffer(mapping, dtype=np.uint8)
    ...
    def touch(self, rows: np.ndarray, pool: ThreadPoolExecutor | None) -> None:
        ...
        def groups(index: np.ndarray) -> list[tuple[np.ndarray, np.ndarray]]:
            ...
            return [
                (self.views[int(g[0])], l)
                for g, l in zip(np.split(s, cuts), np.split(local[index], cuts))
                if g.size
            ]
...
class PagePrefetcher:
    ...
    def __init__(
        ...
        """``input_ids``/``query_start_loc``/``ngram_context`` are the model
        state's fixed input buffers; the pinned staging copies match them."""
        self.tables = tables
